...
cv::Mat disparity_sgm::compute_disparity_method(cv::Mat left, cv::Mat right, float *elapsed_time_ms) {
    ...
    CostAggregationKernelLeftToRight << <
    (rows + PIXELS_PER_BLOCK_HORIZ - 1) / PIXELS_PER_BLOCK_HORIZ, COSTAGG_BLOCKSIZE_HORIZ, 0, stream2 >> >
    ...
    CostAggregationKernelRightToLeft << <
    (rows + PIXELS_PER_BLOCK_HORIZ - 1) / PIXELS_PER_BLOCK_HORIZ, COSTAGG_BLOCKSIZE_HORIZ, 0, stream3 >> >
    ...
    CostAggregationKernelUpToDown << < (cols + PIXELS_PER_BLOCK - 1) / PIXELS_PER_BLOCK, COSTAGG_BLOCKSIZE, 0,
            stream1 >> >
    ...
    CostAggregationKernelDownToUp << < (cols + PIXELS_PER_BLOCK - 1) / PIXELS_PER_BLOCK, COSTAGG_BLOCKSIZE, 0,
            stream1 >> >
...
void disparity_sgm::free_memory() {
...
void disparity_sgm::finish_disparity_method() {
    if (!first_alloc) {
        free_memory();
