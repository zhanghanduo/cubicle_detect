...
namespace Util {
    ...
    HOGFeatureDescriptor::HOGFeatureDescriptor(
            int cell_size, int block_per_cell,
            int n_bins, float angle) :
            CELL(cell_size),
                ...
                if ((rect.x + rect.width <= image.cols) &&
                    ...
                    for (int y = rect.y; y < (rect.y + rect.height); y++) {
                        for (int x = rect.x; x < (rect.x + rect.width); x++) {
                            auto angle = static_cast<float>(Iang.at<float>(y, x));
                            int l_bin;
                            int h_bin;
    ...
    void HOGFeatureDescriptor::computeHOG(
        ...
        if(featureMD.isContinuous()){

            feature_vec.assign((float*)featureMD.datastart, (float*)featureMD.dataend);

        }else{

            for (int i = 0; i < featureMD.rows; ++i){

                feature_vec.insert(feature_vec.end(), featureMD.ptr<float>(i), featureMD.ptr<float>(i)+featureMD.cols);
            }
        }

//        featureMD = featureMD.reshape(1, 1);
//
//        feature_vec.reserve(static_cast<unsigned long>(featureMD.cols));
//
//        memcpy( &(feature_vec[0]), featureMD.data, featureMD.cols * sizeof( float ));
    }
