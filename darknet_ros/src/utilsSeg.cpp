...
void FindFiles(const std::string& dir, const std::string& pattern, std::vector<std::string>& files, bool fullPath)
{
    try {
        files.clear();
...
std::string ChangeExtension(const std::string& fileName, const std::string& newExt)
{
    ...
    if (dotPos == std::string::npos) return fileName + newExt;
