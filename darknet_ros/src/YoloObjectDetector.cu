...
//  recDisparity = static_cast<int **>(calloc(Height, sizeof(int *)));
//  for(ii = 0; ii < Height; ii++)
//      recDisparity[ii] = static_cast<int *>(calloc(disp_size + 1, sizeof(int)));
//
//  recDepth = static_cast<double **>(calloc(Height, sizeof(double *)));
//  for(ii = 0; ii < Height; ii++)
...
//  ObstacleDetector.Initiate(left_info_copy->header.frame_id, disp_size, stereo_baseline_, u0, v0, focal, Width, Height, Scale, min_disparity);
  ObstacleDetector.Initiate(disp_size, stereo_baseline_, u0, v0, focal, Width_crp, Height_crp, Scale, 8,
          camAngle, camHeight, enableNeg, parameter_filename);
}
...
void YoloObjectDetector::publishGrayImage(const cv::Mat& detectionImage, const ros::Publisher& publisher_)
{
	cv_bridge::CvImage cvImage;
//  cvImage.header.stamp = ros::Time::now();
	cvImage.header.stamp = image_time_;
...
//  running_ = 0;
    ...
    duration<double> time_span_det = duration_cast<duration<double>>(t2 - t1);
    classi_duration_ += time_span_det.count();
...
//    disparityFrameOrg = getDepth(left_rectified, right_rectified);
//    for (int r=0; r<disparityFrameOrg.rows; r++) {
//        for (int c = 0; c < disparityFrameOrg.cols; c++) {
//            disparityFrame.at<uchar>(r,c) = recDisparity[r][(int)disparityFrameOrg.at<uchar>(r,c)];
//        }
//    }

    high_resolution_clock::time_point t2 = high_resolution_clock::now();
    duration<double> time_span_stereo = duration_cast<duration<double>>(t2 - t1);
    stereo_duration_ += time_span_stereo.count();
...
//  disparityFrameOrg = cv::Mat(Height_crp, Width_crp, CV_8UC1, cv::Scalar(0));
...
//                    disOrg = static_cast<int>(Util::median_mat(disparityFrameOrg, center_c_, center_r_,
//                                                               median_kernel));
                }
...
void YoloObjectDetector::DisplayResults() {
...
//    publishGrayImage(origLeft, leftImagePub_);
//    publishGrayImage(origRight, rightImagePub_);
...
//		stereo_thread.join();
		if(frame_num % 2 == 0) {
			...
			high_resolution_clock::time_point t2 = high_resolution_clock::now();
			duration<double> time_span_ = duration_cast<duration<double>>(t2 - obs_time_);
			obs_duration_ += time_span_.count();
		}
	}

//	std::cout<<"before obstacle disparity generation"<<std::endl;
	// 7. Get obstacle disparity map by filtering ground and moving objects
...
//	std::cout<<"after display results"<<std::endl;
    ...
    if ( frame_num % 100 == 0 && enableConsoleOutput_ ) {
        ...
        // * 10 means: "times 1000 to make second unit into millisecond unit, and divided by 100 to average"
        whole_duration_ = 0;
        stereo_duration_ = 0;
        obs_duration_ = 0;
        classi_duration_ = 0;
